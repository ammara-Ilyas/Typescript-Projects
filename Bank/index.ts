...
class Customer {
  ...
  addAccount(initialBalance: number): void {
...
async function startBankingSystem() {
  ...
  async function addAccount() {
    ...
    if (customerIndex !== -1) {
      ...
      myBank
        .getCustomers()
        [customerIndex].addAccount(parseInt(initialBalance, 10));
  ...
  async function performTransaction(transactionType: "deposit" | "withdraw") {
    const customerIndex = await getCustomerIndex();
    if (customerIndex !== -1) {
  ...
  async function getCustomerIndex(): Promise<number> {
